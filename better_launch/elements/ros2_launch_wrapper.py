...
class Ros2LaunchWrapper(AbstractNode):
    def __init__(self,
        ...
        self._ros2_launcher = launch.LaunchService(argv=launch_args, noninteractive=True)
    ...
    def _do_start(self) -> None:
        self._launch_process = Process(
            target=self._run_launchservice,
            args=(self._ros2_launcher, self._action_queue),
            name=self.name,
            daemon=True,
        )
        self._launch_process.start()

    def _run_launchservice(self, ros2_launcher, q: Queue) -> None:
        ...
        # add forwarder to launcher asyncio loop
        ros2_launcher.event_loop.create_task(forward_launch_descriptions())
        ros2_launcher.run()
    ...
    def shutdown(self, reason: str, signum: int = signal.SIGTERM) -> None:
        ...
        if self._terminate_requested or signum == signal.SIGKILL:
            self.logger.warning(f"(reason), but {self.name} was asked to terminate before -> escalating to SIGKILL. Killing ROS2 launch service may leave stale processes behind!")
