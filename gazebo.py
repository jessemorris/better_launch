...
def gz_launch_setup(world_file, gz_args = None):
    ...
    if isinstance(world_file, str) and (gz_args is None or isinstance(gz_args, str)):
        ...
        if gz_args is not None:
            full_args += ' ' + gz_args
    '''else:
    full_args = str(world_file) + ' ' + str(gz_args)''' #simple substitution
    ...
    if isinstance(full_args, str):
        ...
        console_msg = logger.warning("This is a warning message")
        if valid_file:
...
class GazeboBridge:
    ...
    def __init__(self, gz_topic, ros_topic, ros_msg, direction, gz_msg = None):
        ...
        if not GazeboBridge.valid(direction):
            logger.error(f'Cannot build ros <-> gz bridge with direction "{direction}": use GazeboBrige.{{gz2ros,ros2gz,bidirectional}}')
            return
    ...
    def yaml(self):
        ...
        return (f'- {GazeboBridge._gz_exec}_topic_name: ', self.gz_topic, '\n',
    ...
    def world():
        ...
        if only_show_args():
            ...
            logger.warn('This launch file will request information on a running Gazebo instance at the time of the launch')
            return GazeboBridge._world_name
        ...
        if GazeboBridge._world_name is None:
            logger.error(f'GazeboBridge: could not find any Gazebo instance (tried {" and ".join(candidates)}), run Gazebo before this launch file.\n                 Read https://github.com/oKermorgant/simple_launch?tab=readme-ov-file#interaction-with-gazebo-sim for more information')
            return
